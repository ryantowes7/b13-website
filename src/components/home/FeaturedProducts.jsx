...
export default function FeaturedProducts() {
  ...
  // Organize products by homePosition with sorting
  const organizeProducts = () => {
    // Filter and sort products by homePosition and homeOrder
    ...
    // Get the most recent one for each position
    const featuredProduct = featured.length > 0 ? featured[0] : null;
    const secondProduct = second.length > 0 ? second[0] : null;
    
    // Carousel includes all carousel products
    const carouselProducts = carousel.length > 0 ? carousel : [];
  ...
  if (!featuredProduct && !secondProduct && carouselProducts.length === 0) {
    return (
      <section className="min-h-screen flex items-center bg-gradient-to-b from-white to-neutral-50 py-12 sm:py-16 lg:py-20">
        <div className="w-full max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-center py-20">
            <p className="text-neutral-500 text-xl">Belum ada produk yang tersedia di homepage</p>
          </div>
        </div>
      </section>
    );
      ...
      <div className="w-full max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {(featuredProduct || secondProduct) && (
          <div className="mb-8 sm:mb-12 lg:mb-16">
            ...
            {featuredProduct && (
              <Link 
                href={`/produk/${featuredProduct.slug}`} 
                className="block lg:float-left lg:w-[66%] lg:pr-4 mb-4 lg:mb-0 group"
              >
                <div className="relative rounded-2xl sm:rounded-3xl overflow-hidden shadow-xl sm:shadow-2xl hover:shadow-3xl transition-all duration-500 transform hover:-translate-y-2" 
                  style={{ 
                    aspectRatio: '4/3', 
                    minHeight: '300px' 
                  }}
                >
                  {/* Mobile: 4:3, Desktop: maintain aspect ratio */}
                  <div className="absolute inset-0">
                    {featuredProduct.home_image || featuredProduct.image ? (
                      <img
                        src={featuredProduct.home_image || featuredProduct.image}
            ...
            {secondProduct && (
              <Link 
                href={`/produk/${secondProduct.slug}`} 
                className="hidden lg:block lg:float-left lg:w-[34%] lg:pl-4 group"
              >
                <div className="relative h-[500px] rounded-3xl overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2">
                  <div className="absolute inset-0">
                    {secondProduct.home_image || secondProduct.image ? (
                      <img
                        src={secondProduct.home_image || secondProduct.image}
          ...
          <div className="relative">
            ...
            >
              ...
              >
                {carouselProducts.map((product, index) => (
                  ...
                  >
                    <div className="relative rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-3"
                      style={{ 
                        height: '280px',
                        minHeight: '280px'
                      }}
                    >
                      <div className="absolute inset-0">
                        {product.home_image || product.image ? (
                          <img
                            src={product.home_image || product.image}
                            alt={product.name}
                            className="w-full h-full object-cover transform group-hover:scale-105 transition-transform duration-700"
                          />
                        ) : (
                      ...
                      <div className="absolute top-3 left-3 right-3 flex justify-between items-start z-10">
                        {product.category && (
                          <span className="inline-flex items-center px-2 py-1 rounded-lg text-xs font-bold bg-primary-500/90 backdrop-blur-md text-white capitalize shadow-lg">
                            {product.category}
                          </span>
                        )}
            ...
            {carouselProducts.length > 1 && (
              <>
                <button
                  onClick={prevSlide}
                  className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-2 sm:-translate-x-4 bg-white shadow-xl rounded-full p-2 sm:p-3 hover:bg-primary-500 hover:text-white transition-all duration-300 transform hover:scale-110 z-10"
                  aria-label="Previous"
                >
                ...
                <button
                  onClick={nextSlide}
                  className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-2 sm:translate-x-4 bg-white shadow-xl rounded-full p-2 sm:p-3 hover:bg-primary-500 hover:text-white transition-all duration-300 transform hover:scale-110 z-10"
                  aria-label="Next"
                >
                ...
                {carouselProducts.map((_, idx) => (
                  <button
                    ...
                    className={`h-2 rounded-full transition-all duration-300 ${
                      currentIndex === idx ? 'w-6 sm:w-8 bg-primary-500' : 'w-2 bg-neutral-300 hover:bg-neutral-400'
                    }`}
